...
- **物理キーボード設定**:
  使用している物理キーボードの種類（基準となるスキャンコードのマッピング）を選択します。デフォルトで用意されている「JIS」や「US」などを選択できるほか、ユーザー自身で独自のマッピング定義を作成することも可能です。
  - 桔梗の実行ファイルと同じ階層にある `map` フォルダ内にテキストファイル（拡張子 `.txt`）を配置すると、そのファイル名がプルダウンの選択肢として追加されます。
  - 自身の環境に合わせて特殊なキー置換をアプリケーションの最下層で行いたい場合などに活用できます。（※書き方の参考として、同フォルダ内に同梱されている `jis106_sample.txt` や `us101_sample.txt` をご覧ください）
...
- **IMEへのアクセス方法**:
  桔梗が内部でWindowsの日本語入力状態を判定する方式を指定します。通常は「自動」のままで問題ありません。特定のソフトなどで桔梗がうまく追従しない場合は変更を試してください。
  - **自動 (TSF → IMM)**: 基本的な設定。TSFを優先し、取得できなければIMMを試して自動判定します。
  - **TSF**: 新しいWindowsアプリ向けのAPIで状態を判定します。
  - **IMM**: 従来型のAPI（IMM32）で判定します。古いソフト等で自動判定が効かない場合に有効です。
  - **無視 (常に有効)**: パソコンの実際のIME状態を確認せず、強制的に常に「日本語入力モード（オン）」として振る舞います。タイピングゲームなどで便利です。
- **ショートカット**:
  桔梗の各種操作キー（ショートカット）を割り当てます。入力欄をクリックしてから設定したいキーの組み合わせを入力してください。BackSpaceキーやDeleteキーで割り当てを解除（なし）にできます。
  - **一時停止 / 再開**: 桔梗の動作を瞬時にオン・オフするためのキーを設定します。ゲームプレイ時など、一時的に通常キーボードに戻したい場合に便利です。
  - **設定画面の起動**: 桔梗の設定画面を開くショートカットキーを設定します。
  - **配列定義の順次切替**: 登録されている配列定義を順番に切り替えるショートカットキーを設定します。

### 2.5. 除外設定

登録したアプリまたはウィンドウが前面にある間、桔梗のキー置換を行わず、物理キー入力をそのままOSへ渡します。桔梗の一時停止、設定画面表示、配列切替のショートカットは除外中も使用できます。

- **アプリ全体を追加**: 直前に使用していたアプリのすべてのウィンドウを除外します。
- **このウィンドウのみ追加**: 直前に使用していたウィンドウだけを、アプリ情報、ウィンドウクラス、タイトル条件で除外します。登録後に「タイトルに含む文字」を短い固定部分へ編集すると、ファイル名などでタイトルが変わるアプリでも安定して一致します。
- **有効スイッチ**: 登録内容を削除せず、一時的にその除外ルールを無効化します。

対象を追加するときは、除外したいアプリを一度前面にしてから桔梗の設定画面へ戻り、追加ボタンを押してください。ウィンドウタイトルやアプリ情報を取得できない場合、その対象は登録できません。
